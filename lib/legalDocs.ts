...
export type LegalBlock =
  ...
  | { kind: 'article'; title: string; body: string }

export type LegalVersion = {
  id: string
  /** UI 표시용 — 예: 2026년 8월 26일 */
  label: string
  blocks: LegalBlock[]
}
...
/** @deprecated 호환용 — article 목록만 */
export type LegalSection = {
  title: string
  body: string
}

const TERMS_V_2026_08_26: LegalVersion = {
  id: '2026-08-26',
  label: '2026년 8월 26일',
  blocks: [
...
const TERMS_V_2026_02_21: LegalVersion = {
  id: '2026-02-21',
  label: '2026년 2월 21일',
  blocks: [
    { kind: 'chapter', title: '제1장 총칙' },
    {
      kind: 'article',
      title: '제1조 (목적)',
      body: '본 약관은 하남이네 힐링펫 서비스 이용과 관련하여 필요한 사항을 정합니다.',
    },
    {
      kind: 'article',
      title: '제2조 (서비스)',
      body: '회사는 마음챙김·펫 케어·대화·감정일기 기능을 제공합니다. 서비스 내용은 운영상 필요에 따라 변경될 수 있습니다.',
    },
  ],
}
...
export const LEGAL_DOCS: Record<LegalDocId, LegalDoc> = {
  terms: {
    id: 'terms',
    title: '이용약관',
    versions: [TERMS_V_2026_08_26, TERMS_V_2026_02_21],
  },
  privacy: {
    id: 'privacy',
    title: '개인정보 처리방침',
...
export function getLegalVersion(
  ...
  if (versionId) {
    const found = doc.versions.find((v) => v.id === versionId)
    if (found) return found
  }
  return doc.versions[0]
}

/** TermsSheet 등 plain text가 필요할 때 — 최신 버전 기준 */
export function legalDocToPlainText(doc: LegalDoc): string {
  const version = getLegalVersion(doc)
  return version.blocks
    .map((b) =>
      b.kind === 'chapter' ? b.title : `${b.title}\n\n${b.body}`,
    )
    .join('\n\n')
}
